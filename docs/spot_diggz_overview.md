...
## 仕様メモ: スポット公開ステータス（approvalStatus）
### 目的
- スポットの信頼性と重複を整理し、検索結果の精度を高める。
### 区分定義
- **approved**: 承認済み（一般ユーザーに公開）。
- **pending**: 承認待ち（申請中）。
- **rejected**: 却下（再申請可）。
- **未申請**: approvalStatus未設定の状態（投稿直後のデフォルト）。
### 昇格フロー（モバイルのみ）
- 投稿したスポットに対し、申請ボタンから承認申請を送る。
- 管理者が重複確認・実在確認後に承認/却下する。
### データモデル
- `approvalStatus`（pending/approved/rejected、未申請はnull）
